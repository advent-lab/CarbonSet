...
def load_tables(dir_name=""):
    ...
    with open(dir_name+"tech_params/gates_perhr_scaling.json",'r') as gph_sc:
        gph_scaling_table = json.load(gph_sc)
    Gates_per_hr_per_core = pd.DataFrame(data=gph_scaling_table,index=tech_indices)
    
    with open(dir_name+"tech_params/beol_feol_scaling.json",'r') as beolfeol_sc:
    ...
    with open(dir_name+"tech_params/dyn_pwr_scaling.json",'r') as dyn_sc:
