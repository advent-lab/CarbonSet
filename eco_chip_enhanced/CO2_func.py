...
#         print("yields:", yields)
    ...
    # print(f"Si_chip: mfgc {mfg_carbon}, area_scale {area_scale} cpa {cpa} area {area} yields {yields}")
...
def Interposer(areas, techs, types, scaling_factors, package_type="passive", always_chiplets=False,
    ...
    if(~np.all(np.array(techs) == techs[0]) or always_chiplets):
        ...
        # print(f"Interposer: {interposer_area}, {np.sum(areas)}")
...
if __name__ == '__main__':
    print("utils: CO2_func.py Running as __main__")
